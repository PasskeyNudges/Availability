...
<p align="center">
  <img src="image/nudges-design.jpg" alt="Display of the UI mockups comparing the three nudge and control groups for each of the five touchpoints. Registration touchpoint: The control group shows a passkey and password button as two equivalent alternatives; the salience nudge highlights the passkey button with a blue background; the visibility nudge shows both options equivalent and explains as text information about passkeys; the choice default nudge shows only one option to create an account, which automatically initiates passkey adoption. Login touchpoint: The control group shows a passkey and password button as two equivalent alternatives; the salience nudge highlights the passkey button with a blue background; the visibility nudge shows both options equivalent and explains as text information about passkeys; the choice default nudge shows only one option to create a passkey, which automatically initiates passkey adoption. Recovery touchpoint: The control group shows a passkey and password button as two equivalent alternatives; the visibility nudge shows both options equivalent and explains as text information about passkeys; the choice default nudge shows only one option to reset the account, which automatically initiates passkey adoption; the effort nudge shows a pop-up asking if a user is sure to use a password again. Settings touchpoint: The control group shows one button to create a passkey with a small blue badge containing 'New'; the salience nudge highlights the passkey button with a blue background; the visibility nudge explains as text information about passkeys; the visibility nudge explains as text information about passkeys; the social reference nudge shows a blue badge containing 'Used by 1,000 users' above the button to create a passkey. Activity touchpoint: The control group shows one button to create a passkey with a small blue badge containing 'New'; the salience nudge highlights the passkey button with a blue background; the visibility nudge explains as text information about passkeys; the reminder nudge shows a pop-up with the information about passkey as a new security feature, containin the passkey and password button as equivalent options." width="65%">
</p>
...
3. Run:
   ...
   ```
5. Outputs will be written to `analysis/output/`.
